...
router.post('/signup', upload1.single('profileImg'), function(req, res) {
    ...
    arr = { "user_name": req.body.user_name, "fullName": req.body.fullName, "userEmail": req.body.userEmail, "profileImg": img_name, "userPassword": req.body.userPassword, "otp": randomnumber }
    ...
    //     })
    User.where
    User.count({ user_name: req.body.user_name, userEmail: email }, function(err, count) {
        if (count == 0) {
...
router.post('/login', upload.single('img'), function(req, res) {
        User.count({ user_name: req.body.user_name, userPassword: req.body.userPassword }, function(err, count) {
            if (count == 1) {
                User.findOne({ name: req.body.user_name, userPassword: req.body.userPassword }, function(err, user) {
                    res.type('application/json');
                    res.setHeader("Content-Type", "application/json");
...
router.post('/addCategory', upload.single('catImg'), function(req, res) {
    ...
    catArr = { 'cat_name': req.body.cat_name, 'catImg': img_name };
...
router.get('/showUploads', upload.single('img'), function(req, res) {
    ...
    Image.find().limit(2).skip(2).exec(function(err, img_list) {
        //res.send(img_list);
...
router.post('/imgUpload', upload.single('img'), function(req, res) {
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        // Category.find(query, function(err, list) {
        //     for (var c = 0; c <= list.length; c++)
        //         console.log(list[c].cat_name);
        // })
...
router.post('/editProfile', upload1.single('profileImg'), function(req, res) {
    arr = Array();
    arr = { "fullName": req.body.fullName, "profileImg": img_name }
...
router.post('/likeImg', upload.single('img'), function(req, res) {
    res.type('application/json');
    res.setHeader("Content-Type", "application/json");
...
router.post('/Feeds', upload.single('img'), function(req, res) {
    ...
    if (req.body.user_id != "") {
        ...
        if (action == "myUploads") {
            Image.find().where('user_id').equals(req.body.user_id).exec(function(err, result) {
                res.json({ response: 1, data: result, message: "IMage List" });
            })
        }
        ...
        if (action == "myCategory") {
            ...
            user_cate.find({}).select({ 'user_id': 1, 'cate_id': 1 }).where('user_id').equals(id).exec(function(err, result) {
                ...
                var unique = cArr.filter(function(elem, index, self) {
                    ...
                    //     console.log(unique);
                if (result.length == 0) {
                    Image.find().limit(20).skip(1).exec(function(err, img_list) {
                        res.json({ response: 1, data: img_list, message: "IMage List" });
                    })
                } else {
                    ...
                    cat_img.find().where('cat_id').in(unique).exec(function(er, result) {
                        ...
                        Image.find().where('img').in(imgArr).exec(function(err, result) {
                            res.json({ response: 1, data: result, message: "IMage List" });
                        })
    ...
    if (req.body.user_id == '') {
        Image.find().limit(10).skip(1).exec(function(err, img_list) {
            res.json({ response: 1, data: img_list, message: "IMage List" });
        })
    }

});
router.post('/categoryThumb', function(req, res) {
    cArrName = Array();
    data1 = {};
    cArr = Array();
    Category.find(function(err, resu) {
        for (var t = 0; t < resu.length; t++) {
            //               key = resu[t].cat_name;
            // data1[resu[t].cat_name] = Image.findOne({ "category": { "$regex": resu[t]._id, "$options": "i" } }).exec(function(err, img_list1) {
            //     data1["resu[t].cat_name"] = img_list1;
            // });
            data1[resu[t].cat_name] = catImg(resu[t]._id)

        }

        res.json({ response: 1, data: data1, message: "IMage List" });
    });
})

function catImg(e) {
    test = Image.findOne({ "category": { "$regex": e, "$options": "i" } }).stream();
    console.log(test);
}
...
router.post('/chooseCategory', upload.single('img'), function(req, res) {
    ...
    var update = { $set: { 'selectedCategory': true } }
    var options = { new: true };
    User.findByIdAndUpdate(query, update, options, function(err, user) {
        if (err) {
            console.log('got an error');
...
router.post('/cImg', upload.single('img'), function(req, res) {
    Image.find().limit(10).skip(15).exec(function(err, img_list) {
        res.json({ response: 1, data: img_list, message: "IMage List" });
    })
})
...
// image object in Share image,Download image and like image,
// for fbLogin and gplusLogin59ae43bd9a840a078ccf1e28