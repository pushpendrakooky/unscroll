...
const myadmin = require('../models/myadmin');
...
router.post('/signup', upload1.single('profileImg'), function(req, res) {
    ...
    if (img == '') {
        ...
        arr = { "user_name": req.body.user_name, "fullName": req.body.fullName, "userEmail": req.body.userEmail, "userPassword": req.body.userPassword, "otp": randomnumber }
...
router.post('/forgetPassword', upload.single('img'), function(req, res) {
        var email = req.body.userEmail;
        var otp = Math.floor(Math.random() * 90000) + 10000;
        User.findOne({ userEmail: email }, function(err, count) {
            if (!count) {
                res.type('application/json');
                res.setHeader("Content-Type", "application/json");
                res.json({ response: 0, data: [], message: "Email id Not Exist" });
            } else {
                //   console.log(count._id);
                var query = { "_id": count._id };
                var update = { $set: { otp: otp } }
                var options = { new: true };
                User.findByIdAndUpdate(query, update, options, function(err, user) {
                    if (err) {
                        ...
                        res.type('application/json');
                        res.setHeader("Content-Type", "application/json");
                        res.json({ response: 1, data: user, message: "OTP send to registered email" });
                    }
                    //    return person;
                });
    ...
    //delete account 
router.post('/deleteAccount', upload.single('img'), function(req, res) {
    user_id = req.body.user_id;
    var query = { "_id": user_id };
    var update = { $set: { accountStatus: false } }
    var options = { new: true };
    User.findByIdAndUpdate(query, update, options, function(err, user) {
        if (err) {
            ...
            // res.setHeader("Content-Type", "application/json");
            // res.json({ response: 1, data: user, message: "OTP send to registered email" });
            User.findOne({}).select({ 'user_id': 1, 'accountStatus': 1, 'user_name': 1 }).where('user_id').equals(user_id).exec(function(err, result) {
                //    User.findById({ _id: user_id }, function(err, data) {
                res.type('application/json');
                res.setHeader("Content-Type", "application/json");
                res.json({ response: 1, data: result, message: "Deleted" });
            });
        }
...
router.post('/login', upload.single('img'), function(req, res) {
    User.count({ user_name: req.body.user_name, userPassword: req.body.userPassword }, function(err, count) {
        if (count == 1) {
            User.findOne({ user_name: req.body.user_name, userPassword: req.body.userPassword, accountStatus: "true" }, function(err, user) {
                if (!user) {
                    res.type('application/json');
                    res.setHeader("Content-Type", "application/json");
                    res.json({ response: 0, data: {}, message: " Account deactivated" });
                } else {
                    res.type('application/json');
                    res.setHeader("Content-Type", "application/json");
            ...
            res.type('application/json');
            res.setHeader("Content-Type", "application/json");
            res.json({ response: 0, data: {}, message: "Invalid login" });
...
//All User
router.post('/allUser', upload.single('img'), function(req, res) {
    var user_id = req.body.user_id;
    User.find({}).select({ 'user_id': 1, 'name': 1, 'fullName': 1, 'profileImg': 1, "_id": 1 }).where("_id").ne(req.body.user_id).exec(function(err, result) {
        res.type('application/json');
        res.setHeader("Content-Type", "application/json");
...
router.post('/categoryList', upload.single('img'), function(req, res) {
    if (req.body.user_id == '') {
        ...
        user_cate.find().where('user_id').equals(req.body.user_id).exec(function(er, result) {
            ...
            Category.update({ "_id": { "$nin": unique } }, { categoryLike: false }, { multi: true }, function(err, person) {
                //     console.log(person);
            });
            Category.find().exec(function(err, categoryList) {
                res.type('application/json');
...
router.get('/showUploads', upload.single('img'), function(req, res) {
    Image.find({}).sort({ img_addedOn: 'descending' }).exec(function(err, img_list) {
        //res.send(img_list);
...
router.post('/imgUpload', upload.single('img'), function(req, res) {
    ...
    arr = {
        ...
        'share': length,
        'img_url': 'localhost:3000/Images/' + img_name,
        'thumb_url': 'localhost:3000/thumbnail/' + img_name
    }
...
router.post('/editProfile', upload1.single('profileImg'), function(req, res) {
    arr = Array();
    arr = { "fullName": req.body.Name, "profileImg": 'http://ec2-54-89-168-230.compute-1.amazonaws.com:3000/Profile/' + img_name, "userPassword": req.body.userPassword }
    var query = { "_id": req.body.user_id };
    var update = { $set: { "fullName": req.body.fullName, "userPassword": req.body.userPassword, profileImg: 'http://ec2-54-89-168-230.compute-1.amazonaws.com:3000/Profile/' + img_name } }
    var options = { new: true };
    User.findByIdAndUpdate(query, update, options, function(err, person) {
        var data = Array();
        res.type('application/json');
...
router.post('/feeds', upload.single('img'), function(req, res) {
    ...
    if (action == "search") {
        ...
        //   var newstr = haa.replace(' ', '#', );
        var newstr = haa.replace(/ /g, "#");
        //   console.log(newstr);
        Image.find({ description: new RegExp(newstr, "i") }, function(err, doc) {
            res.json({ response: 1, data: doc, message: "IMage List" });
        ...
        if (haa.length > 0) {
            ...
            //         Image.find({}).sort({ img_addedOn: 'descending' }).where('img').in(unique).exec(function(err, result) {
            ...
            Image.find({}).limit(20).skip(index * 20).exec(function(err, img_list) {
                res.json({ response: 1, data: img_list, message: "IMage List" });
                //    res.send(JSON.stringify(img_list, null, 4));
            })
        }
    }
    if (req.body.user_id != "") {
        ...
        if (action == "invite") {
            iarr = Array();
            // Image.find({}).select({ 'img': 1, 'img_url':1,'thumb_url':1,'user_id': 1, 'description': 1 }).where('img').in(iarr).limit(20).skip(index * 20).exec(function(err, result) 
            User.findOneAndUpdate({ "_id": id }, { "$set": { "totalInvite": 0, "lastView": 0 } }).exec(function(err, book) {
                if (err) {
                    console.log(err);
            ...
            img_invitation.find().where('user_id').equals(id).exec(function(err, list) {
                ...
                Image.find().where('img').in(iarr).limit(20).skip(index * 20).exec(function(err, result) {
                    res.json({ response: 1, data: result, message: "IMage List" });
                })
            })
        }
        ...
        if (action == "likes") {
            ...
            img_like.find().where('user_id').equals(req.body.user_id).exec(function(err, data) {
                ...
                Image.find().where('image_id').nin(narr).limit(20).exec(function(err, r) {});
            })
        }
        //for myLike
    ...
    if (req.body.user_id == 0 && action != "search") {

        Image.update()
        Image.update({}, { 'iLike': false }, { multi: true }, function(err, person) {
            console.log(person);
        })
...
function HaArr(e) {
    var unique = '';
    hArr = Array();
    hash.find({ hash_name: new RegExp('^' + e + '$', "i") }, function(err, doc) {
        //Do your action here..
        if (err) return err;
        for (var h = 0; h < doc.length; h++)
            hArr.push(doc[h].img_name);
        unique = hArr.filter(function(elem, index, self) {
            return index == self.indexOf(elem);
        })
    });
...
router.post('/BadgeInfo', upload.single('img'), function(req, res) {
    user_id = req.body.user_id;
    // console.log(user_id);
    User.findOne({}).select({ 'user_id': 1, 'lastView': 1, 'totalInvite': 1 }).where('user_id').equals(user_id).exec(function(err, result) {
        //    User.findById({ _id: user_id }, function(err, data) {
        res.type('application/json');
        res.setHeader("Content-Type", "application/json");
        res.json({ response: 0, data: result, message: " Created" });
    });
})
...
router.post('/adminLogin', function(req, res) {
    ...
    arr = { 'admin_name': req.body.userName, 'admin_password': pass };

    var data;
    myadmin.count({ 'admin_name': req.body.userName, 'admin_password': pass }, function(err, data) {
        if (data > 0) {
            console.log("Hello");
            //      res.redirect('../api/imgGallery');
...
router.get('/imgGallery', function(req, res) {
    Image.find({}).sort({ img_addedOn: 'descending' }).exec(function(err, img_list) {
        ...
        if (img_list.length == 0) {
            ...
            Image.find({}).exec(function(err, i) {
...
router.get('/allCategory', function(req, res) {
    Category.find().exec(function(err, categoryList) {
        ...
        // res.setHeader("Content-Type", "application/json");
        // res.json({ response: 1, data: categoryList, message: " Category List" });
        res.render('category.ejs', { category: categoryList })
    });
})
router.get('/imgGallery1', function(req, res) {
    Image.find({}).sort({ img_addedOn: 'descending' }).exec(function(err, img_list) {
        //res.send(img_list);
        if (img_list.length == 0) {
            res.type('application/json');
...
router.get('/userList', function(req, res) {
    User.find().exec(function(err, userL) {
        res.render('users.ejs', { userL: userL })
    });
})
