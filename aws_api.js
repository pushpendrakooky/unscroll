...
router.post('/signup', upload1.single('profileImg'), function(req, res) {
    ...
    arr = { "user_name": req.body.user_name, "fullName": req.body.fullName, "userEmai                                                                                                                l": req.body.userEmail, "profileImg": 'http://ec2-54-89-168-230.compute-1.amazona                                                                                                                ws.com:3000/Profile/' + img_name, "userPassword": req.body.userPassword, "otp": random number }

    res.send
    var randomnumber = Math.floor(Math.random() * 90000) + 10000;
    //  console.log(randomnumber);

    data = { 'user_name': req.body.user_name, 'userEmail': email, 'otp': randomn umber };
    ...
    User.count({ user_name: req.body.user_name }, function(err, count) {
        if (count == 0) {
            User.create(arr).then(function(user) {
                ...
                User.findByIdAndUpdate(query, update, options, function(err, use r) {
                    if (err) {
                        //console.log('got an error');
                    } else {
                        res.type('application/json');
                        res.setHeader("Content-Type", "application/json");
                        res.json({ response: 1, data: user, message: "Account Cr                                                                                                                eated" });
                    }
                    //    return person;
                });
                //    res.send(user);
            ...
            res.type('application/json');
            res.setHeader("Content-Type", "application/json");
            res.json({ response: 0, data: 'user', message: " Account Already Cre                                                                                                                ated" });
...
router.post('/login', upload.single('img'), function(req, res) {
    User.count({ user_name: req.body.user_name, userPassword: req.body.userPassw ord }, function(err, count) {
        if (count == 1) {
...
router.post('/fbLogin', upload.single('Img'), function(req, res) {
    ...
    if (req.body.fbId != '') {
        ...
        User.count({ fbId: req.body.fbId }, function(err, count) {
            if (count == 0) {
                data['response'] = 1;
                arr = { "user_name": req.body.user_name, "fullName": req.body.fullName, "fbId": re q.body.fbId, "profileImg": req.body.profileImg };
                User.create(arr).then(function(user) {
                    // res.type('application/json');
                    //res.setHeader("Content-Type", "application/json");
                    //res.json({ response: 1, data: user, message: " Account Cre                                                                                                                ate" });
                    var query = { "_id": user._id };
                    var update = { $set: { user_id: user._id } }
                    var options = { new: true };
                    User.findByIdAndUpdate(query, update, options, function(err, user) {
                        if (err) {
                            ...
                            res.type('application/json');
                            res.setHeader("Content-Type", "application/json");
                            res.json({ response: 0, data: user, message: "Accoun                                                                                                                t Created" });
                        }
                        //    return person;
                    });
                });
    ...
    if (req.body.user_id == '') {
        ...
        Category.find().exec(function(err, categoryList) {
            ...
            Category.update({ "_id": { "$in": categoryList } }, { categoryLike: false }, { multi: true }, function(err, person) {
                ...
                /* Category.update({ "_id": { "$nin": unique } }, { categoryLike: false }, { mul                                                                                                                ti: true }, function(err, person) {
                                console.log(person);
                            });*/
        ...
        user_cate.find().where('user_id').equals(req.body.user_id).exec(function(er, result) {
            ...
            Category.update({ "_id": { "$nin": unique } }, { categoryLike: false }, { mult i: true }, function(err, person) {
                //     console.log(person);
            });
            Category.find().exec(function(err, categoryList) {
                res.type('application/json');
...
router.get('/showUploads', upload.single('img'), function(req, res) {
    Image.find({}).sort({ img_addedOn: 'descending' }).exec(function(err, img_ list) {
        //res.send(img_list);
...
function img_count(cid, callback) {
    Image.find({ "category": { "$regex": cid, "$options": "i" } }).count(functio n(err, img_list1) {
...
router.post('/imgUpload', upload.single('img'), function(req, res) {
    ...
    thumb({
        source: 'Images/' + img_name, // could be a filename: dest/path/image.jp                                                                                                                g
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        Category.findByIdAndUpdate(query, { "$push": { "img_url": 'http://ec2-54-89-168-                                                                                                                230.compute-1.amazonaws.com:3000/thumbnail/' + img_name } }, options, function(e rr, lo) {
            //            console.log(lo);
        });
    ...
    for (var a = 1; a < tag.length; a++) {
        ta = { 'hash_name': tag[a], 'img_name': img_name, 'user_id': req.body.us er_id };
    ...
    arr = {
        ...
        'thumb_url': 'http://ec2-54-89-168-230.compute-1.amazonaws.com:3000/thumb                                                                                                                nail/' + img_name
    }
    Image.create(arr).then(function(image) {
        ...
        Image.findByIdAndUpdate(query, update, options, function(err, image) {
            if (err) {
                ...
                res.type('application/json');
                res.setHeader("Content-Type", "application/json");
                res.json({ response: 1, data: image, img_path: __dirname + "/Ima                                                                                                                ges/" + img_name, thumb_path: __dirname + "/thumbnail/" + img_name, message: " I                                                                                                                mage Upload Success" });
            }
            //    return person;
        });
    })
...
router.post('/searchHash', upload.single('img'), function(req, res) {
    ...
    var data = Array();
    hash.find({ hash_name: new RegExp('^' + req.body.hash + '$', "i") }, functio n(err, doc) {
        //Do your action here..
        if (err) {
...
router.post('/downloadImage', upload.single('img'), function(req, res) {
    ...
    Image.findByIdAndUpdate(query, update, options, function(err, person) {
        if (err) {
            ...
            res.json({ response: 1, data: person, message: " Image Download Succ                                                                                                                ess" });
        }
    })
})
...
router.post('/OtpVerify', upload.single('img'), function(req, res) {
    ...
    User.findOne({ userEmail: email, otp: otp }, function(err, found) {
        ...
        //     Image.findByIdAndUpdate(query, update, options, function(err, per                                                                                                                son) {
        //         if (err) {

        //             res.json({ response: 0, data: "fail", message: " Otp Not                                                                                                                 Verify" });
        //         } else {

        //             res.json({ response: 1, data: req.body, message: " OTP Ve                                                                                                                rify Success" });
        //         }
        //     })
...
router.post('/resetPassword', upload.single('img'), function(req, res) {
    ...
    User.findByIdAndUpdate(query, update, options, function(err, person) {
        if (err) {
            ...
            res.json({ response: 0, data: "fail", message: " Password Not Chaang                                                                                                                e" });
        } else {

            res.json({ response: 1, data: req.body, message: " Password Verify C                                                                                                                hange" });
        }
    })
...
router.post('/editProfile', upload1.single('profileImg'), function(req, res) {
    arr = Array();
    arr = { "fullName": req.body.Name, "profileImg": 'http://ec2-54-89-168-230.c                                                                                                                ompute-1.amazonaws.com:3000/Profile/' + img_name }
        //console.log(img_name);
    var query = { "_id": req.body.user_id };
    var update = { $set: { "fullName": req.body.fullName, profileImg: 'http://ec                                                                                                                2-54-89-168-230.compute-1.amazonaws.com:3000/Profile/' + img_name } }
    var options = { new: true };
    User.findByIdAndUpdate(query, update, options, function(err, person) {
        var data = Array();
        res.type('application/json');
...
router.post('/imgLike', upload.single('img'), function(req, res) {
    ...
    //console.log(req.body);
    img_like.count({ img_id: req.body.img_id, user_id: req.body.user_id }, funct ion(err, count) {
        ...
        if (count == 0 || req.body.action == "Like") {
            ...
            Image.findByIdAndUpdate(query, update, options, function(err, person) {
                if (err) {
                    ...
                    res.json({ response: 1, data: person, message: " Like Succes                                                                                                                s" });
                }
            })
            ...
            Image.findByIdAndUpdate(query, update, options, function(err, person) {
                if (err) {
                    ...
                    res.json({ response: 0, data: "fail", message: " Image Downl                                                                                                                oadding" });
                } else {

                    res.json({ response: 1, data: person, message: " Unlike Succe                                                                                                                ss" });
                }
            });
            img_like.findOneAndRemove({ user_id: req.body.user_id, img_id: req.b ody.img_id }).then(function(image) {
                //res.send(image);
            });
...
router.post('/feeds', upload.single('img'), function(req, res) {
    ...
    if (action == "search") {
        hArr = Array();
        hash.find({ hash_name: new RegExp('^' + req.body.hash + '$', "i") }, fun ction(err, doc) {
            //Do your action here..
            if (err) return err;
            ...
            Image.find({}).sort({ img_addedOn: 'descending' }).where('img').in(unique).limit(20).skip(index * 20).exec(function(err, result) {
    ...
    if (req.body.user_id != "") {
        ...
        if (action == "invite") {
            ...
            img_invitation.find().where('user_id').equals(id).exec(function(err, list) {
                ...
                Image.find({}).select({ 'img': 1, 'img_url': 1, 'thumb_url': 1, 'use                                                                                                                r_id': 1, 'description': 1 }).where('img').in(iarr).limit(20).skip(index * 20).e xec(function(err, result) {
                    res.json({ response: 1, data: result, message: "IMage List" });
                })
            })
        }
        ...
        if (action == "likes") {
            narr = Array();
            img_like.find().where('user_id').equals(req.body.user_id).exec(funct ion(err, data) {
                for (var p = 0; p < data.length; p++) {
                    narr.push(data[p].img_id);
                }
                Image.find({}).sort({ img_addedOn: 'descending' }).where('image_                                                                                                                id').in(narr).limit(20).skip(index * 20).exec(function(err, result) {
                    res.json({ response: 1, data: result, message: "IMage List" });
                })
            })
        }
        ...
        if (req.body.myLikes == 1) {
            arr = Array();
            img_like.find().where('user_id').equals(id).exec(function(err, likeL ist) {
                for (var l = 0; l < likeList.length; l++) {
                    Image.find().where('image_id').equals(likeList[l].img_id).ex ec(function(err, lList) {
                        data['img'] += list
                    })
        ...
        if (action == "myUploads") {
            Image.find().where('user_id').equals(req.body.user_id).limit(20).ski p(index * 20).exec(function(err, result) {
                res.json({ response: 1, data: result, message: "IMage List" });
            })
        }
        if (action == "myCategory") {
            ...
            var cArr = Array();
            var imgArr = Array();
            user_cate.find({}).select({ 'user_id': 1, 'cate_id': 1 }).where('use                                                                                                                r_id').equals(id).exec(function(err, result) {
                for (var c = 0; c < result.length; c++)
                ...
                cat_img.find().where('cat_id').in(unique).exec(function(er, resu lt) {
                    //    res.json({ response: 1, data: result, message: "IMage                                                                                                                 List" });
                    for (i = 0; i < result.length; i++)
                        imgArr.push(result[i].img_name);
                    //        Image.find().where('img').in(imgArr).limit(20).skip(index * 20).exec(f                                                                                                                unction(err, result) {

                    Image.find().where('img').in(imgArr).limit(20).skip(index * 20).exec(function(err, result) {
                        res.json({ response: 1, data: result, message: "IMage Li                                                                                                                st" });
                    })
    ...
    if (req.body.user_id == 0) {
        Image.find({}).sort({ img_addedOn: 'descending' }).limit(20).skip(index * 20).exec(function(err, img_list) {
            res.json({ response: 1, data: img_list, message: "IMage List" });
        })
    }
...
router.post('/chooseCategory', upload.single('img'), function(req, res) {
    ...
    user_cate.remove({ user_id: req.body.user_id }, function(err, de) {
        if (err) throw err;
